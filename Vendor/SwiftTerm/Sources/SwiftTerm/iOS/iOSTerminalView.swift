...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    */
    ...
    // Korean IMEs can reinterpret a final consonant as the initial consonant
    // of the next syllable when a vowel follows. For example, "핫" + "ㅔ"
    // must replace the last syllable with "하세", not discard "하" and send
    // only "세". Keep the local input buffer and the remote PTY in lockstep.
    private func tryResyllabifyKoreanFinalBeforeVowel(_ text: String) -> Bool {
        guard let language = textInputMode?.primaryLanguage, language.hasPrefix("ko") else { return false }
        guard _markedTextRange == nil else { return false }
        guard _selectedTextRange.isEmpty,
              _selectedTextRange.endPosition.offset == textInputStorage.textInputUTF16Count else { return false }
        guard text.count == 1, let vowel = text.first else { return false }
        guard let lastCharacter = textInputStorage.last,
              let replacement = SwiftTermHangulInput.resyllabifyFinalConsonant(
                base: lastCharacter,
                followingVowel: vowel
              ) else { return false }

        uitiLog("koreanResyllabifyFinal base:\(lastCharacter) vowel:\(vowel) -> \(replacement)")
        ...
        _selectedTextRange = TextRange(
            from: TextPosition(offset: newOffset),
