...
`stg` is the TestFlight release branch. GitHub Actions runs the `iOS CI` simulator test job for every pull request and every `stg` push. Xcode Cloud is the distribution authority: it runs a second test action, archives the app, and publishes the archive to TestFlight only when those Xcode Cloud actions succeed.
...
4. Create a workflow named `TestFlight (main)` with these settings:
   - Start condition: branch changes on `stg`.
...
5. Start the first build from Xcode. After it succeeds, the next `stg` push automatically creates the next TestFlight build.
