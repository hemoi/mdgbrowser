...
final class BrowserSession: NSObject, WKNavigationDelegate, WKUIDelegate {
    ...
    func clearDeveloperConsole() async throws {
        _ = try await webView.evaluateJavaScript("window.__retoDevtools && (window.__retoDevtools.logs = [])")
    }
    ...
    (() => {
      if (window.__retoDevtools) return;
      const state = { logs: [] };
      const render = value => {
    ...
    (() => JSON.stringify({
      ...
      logs: (window.__retoDevtools && window.__retoDevtools.logs) || [],
    ...
    (() => {
      if (document.getElementById('reto-reader-style')) return true;
      const style = document.createElement('style');
      style.id = 'reto-reader-style';
      style.textContent = `
    ...
    (() => { document.getElementById('reto-reader-style')?.remove(); return true; })()
    """#
}
