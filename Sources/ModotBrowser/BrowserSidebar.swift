...
struct BrowserSidebar: View {
            ...
            if store.activeWorkspace.splitEnabled {
                HStack(spacing: 10) {
                    ...
                    ForEach([0.35, 0.5, 0.65], id: \.self) { ratio in
                        ...
                        .foregroundStyle(isSelectedRatio(ratio) ? theme.background : theme.mutedLabel)
                        .frame(width: 34, height: 24)
                        .background(isSelectedRatio(ratio) ? theme.accent : Color.clear)
                        .overlay {
                            RoundedRectangle(cornerRadius: 6)
                                .stroke(theme.border, lineWidth: 0.75)
                        }
                        .clipShape(RoundedRectangle(cornerRadius: 6))
                        .accessibilityLabel("Set left pane to \(Int(ratio * 100)) percent")
                    }
                }
                .padding(.horizontal, 14)
                .frame(height: 38)
    ...
    private func isSelectedRatio(_ ratio: Double) -> Bool {
        abs(store.activeWorkspace.splitRatio - ratio) < 0.01
    }
}
