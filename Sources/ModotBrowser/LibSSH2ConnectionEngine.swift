...
final class LibSSH2ConnectionEngine: SSHConnectionEngineProtocol, @unchecked Sendable {
    ...
    private static let runtimeResult: Int32 = libssh2_init(0)
    ...
    func run(
        ...
        guard Self.runtimeResult == 0 else {
    ...
    private func runSession(
        ...
        let socket = try connectSocket(host: profile.host, port: profile.port)
        setSocket(socket)
        ...
        try authenticate(session: session, profile: profile)
        ...
        setEnvironment("LANG", value: "en_US.UTF-8", channel: openedChannel)
        setEnvironment("LC_CTYPE", value: "en_US.UTF-8", channel: openedChannel)
    ...
    private func authenticate(session: OpaquePointer, profile: SSHProfile) throws {
        ...
        if result != 0, available.contains("keyboard-interactive") {
            keyboardSecret.set(password)
            defer { keyboardSecret.set(nil) }
            result = username.withCString { usernamePointer in
    ...
    private func hostKeyFingerprint(session: OpaquePointer) throws -> String {
    ...
    private func setEnvironment(_ name: String, value: String, channel: OpaquePointer) {
    ...
    private func socketBecameWritable(_ descriptor: Int32) -> Bool {
