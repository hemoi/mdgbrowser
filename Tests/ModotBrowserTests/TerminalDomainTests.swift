...
final class TerminalDomainTests: XCTestCase {
    ...
    func testTerminalViewSendsComposedKoreanAndCJKAsUTF8() {
        let view = ModotTerminalView(frame: .zero)
        var sent: [UInt8] = []
    ...
    func testKoreanFinalConsonantMovesToFollowingVowelWithoutDroppingSyllable() {
        XCTAssertEqual(
            ModotTerminalView.resyllabifyKoreanForTesting(base: "핫", followingVowel: "ㅔ"),
            "하세"
        )
        XCTAssertEqual(
            ModotTerminalView.resyllabifyKoreanForTesting(base: "읽", followingVowel: "ㅓ"),
            "일거"
        )
        XCTAssertNil(ModotTerminalView.resyllabifyKoreanForTesting(base: "하", followingVowel: "ㅔ"))
    }
